...
their trade's classification) by materiality bucket, category, direction, fill
quality, and exit reason. Pure reads — it must never modify the DB."""
import pytest
...
def _seed(tmp_db, rows):
    ...
    so tests can exercise NULLs and odd values without the full open path."""
    conn = tmp_db._conn()
    for i, r in enumerate(rows, start=1):
        conn.execute(
            """INSERT INTO trades (id, market_id, market_question, claude_score,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'executed', ?, ?)""",
            (i, f"cond{i}", f"Market {i}?", 0.7, 0.5, 0.2, r.get("side", "YES"),
        ...
        conn.execute(
            """INSERT INTO positions (trade_id, condition_id, market_question, side,
                                      ...
                                      actual_cost_usd)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (i, f"cond{i}", f"Market {i}?", r.get("side", "YES"), 0.5,
             r.get("amount_usd", 20.0), r.get("status", "closed_manual"),
             r.get("realized_pnl_usd"), r.get("exit_reason"), r.get("category"),
